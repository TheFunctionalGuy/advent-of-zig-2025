...
pub const Rotation = struct {
    pub fn parse(tokens: []const u8) !isize {
        ...
        var parsed_distance = try std.fmt.parseInt(isize, distance, 10);
...
fn Dial(comptime count_type: CountingType) type {
    return struct {
        ...
        pub fn apply(self: *Self, rotation: isize) void {
            ...
            switch (count_type) {
                CountingType.resting => {
                    if (updated_position == 0) {
                        self.times_at_zero += 1;
                    }
                },
                CountingType.visiting => {
...
pub const RestingDial = Dial(CountingType.resting);
pub const VisitingDial = Dial(CountingType.visiting);
